...
export async function showIter() {
    ...
    querySnapshot.forEach((doc) => {
        ...
        if (cont >= querySnapshot.size) {
            cad += `<div id="iter${cont}" class="rounded border border-dark mt-3 bg-iter" style="height: 70px; margin-bottom: 100px">`
        } else {
            cad += `<div id="iter${cont}" class="rounded border border-dark mt-3 bg-iter" style="height: 70px">`
        }
        cad += `<table style="height: 70px">
...
export async function getIter(iter) {
    ...
    let qGastos = query(gastosRef, where("iterId", "==", iter.iterId))
    ...
    querySnapshot.forEach(async (doc) => {
        ...
        //Añade un margen inferior en caso de ser el último gasto
        if (cont >= querySnapshot.size) {
            cad += `<div id="${doc.data().gastoId}" class="rounded border border-dark mt-3 bg-iter" style="padding: 10px; margin-bottom: 100px">`
        } else {
